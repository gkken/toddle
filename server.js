...
app.post("/todd", (req, res) => {
  ...
  db.query(
    sql,
    [`${babyTodd}`, `${plainTextTodd}`, `${userId}`],
    (err, dbRes) => {
      db.query(
        `UPDATE users SET todd_count = todd_count + 1 WHERE userId = ${userId}`,
        (req, res) => {}
      );
      res.redirect("/home");
    }
...
app.post("/edit-profile", (req, res) => {
  ...
  db.query(
    sql,
    [req.body.name, req.body.bio, req.body.pfp, req.session.userId],
    (err, dbRes) => {
...
app.delete("/follow-unfollow", (req, res) => {
  ...
  db.query(sql, [req.session.userId, req.body.id], (err, dbRes) => {
    db.query(
      `UPDATE users SET following_count = following_count - 1 WHERE userid = $1`,
      [req.session.userId],
      (err, dbRes) => {
        db.query(
          `UPDATE users SET follower_count = follower_count - 1 WHERE userid = $1`,
          [req.body.id],
          (err, dbRes) => {}
        );
      }
